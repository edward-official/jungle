function q1(n) {
  value = 0;
  for(let traverse=1; traverse<=n; traverse++) value += traverse;
  return value;
}
console.log(q1(10));
...
function q2(criteria) {
  for(let index=0; index<mise_list.length; index++) {
    if(mise_list[index].IDEX_MVL<criteria) {
      console.log(mise_list[index].MSRSTE_NM + ": " + mise_list[index].IDEX_MVL);
    }
  }
}
q2(40);
...
function q3(criteria) {
  for(let index=0; index<bikes.length; index++) {
    if(bikes[index].parkingBikeTotCnt<=criteria) {
      console.log(bikes[index].stationName);
    }
  }
}
q3(5);