...
def show_movies():
  ...
  if sortMode == 'likes':
    movies = list(db.movies.find({'trashed': False}, {}).sort('likes', -1))  # 정렬 추가됨
  else:
...
def like_movie():
  ...
  movie = db.movies.find_one({'trashed': False})  # 예시로 조건 추가
  ...
  # 4. 하나의 영화만 영향을 받아야 하므로 result.modified_count 가 1이면  result = success 를 보냄
